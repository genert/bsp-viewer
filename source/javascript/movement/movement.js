...
export default class {
  ...
  accelerate (dir, speed, accel) {
    var currentSpeed = vec3.dot(this._velocity, dir);
    var addSpeed = speed - currentSpeed;
    if (addSpeed <= 0) {
      return;
    }

    var accelSpeed = accel*MOVEMENT_FRAMETIME*speed;
  ...
  stepSlideMove (gravity) {
    ...
    if ( this._velocity[2] > 0 && (trace.fraction == 1.0 || vec3.dot(trace.plane.normal, up) < 0.7)) { return; }
